...
def end_to_end_execute_workflow_handler(msg):
    """End to end execute workflow handler."""
    ...
    assert all(
        [workflow_to_execute, workflow_process_id, process_id]
...
def end_to_end_check_handler(msg):
    ...
    assert all(
        [endpoint, expect, description, process_id]
